...
final class TestController
{
    ...
    public function myEndpoint(): JsonResponse
    {
        ...
        return new JsonResponse(['success' => true, 'url' => $this->request->getUri()->__toString(), 'language' => $languageId]);
    }

    #[AsApiEndpoint(method: 'POST', path: '/api/v1/my-post-endpoint')]
    public function someOtherEndpoint()
    {
        $languageId = $this->context->getPropertyFromAspect('language', 'id');
    ...
    public function someParamEndpoint(int $param1, string $param2)
    {
        $languageId = $this->context->getPropertyFromAspect('language', 'id');

        return new JsonResponse(['success' => true, 'url' => $this->request->getUri()->__toString(), 'language' => $languageId, 'parameters' => [$param1, $param2]]);
    }
}
